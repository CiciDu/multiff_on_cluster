...
def _cv_dev_poisson(y: np.ndarray,
    ...
    for hold in folds:
        ...
        train = ~test
        res = _fit_glm(y[train], X.iloc[train], offset_log if np.isscalar(offset_log) else offset_log[train])
...
def forward_block_select(df_X_full: pd.DataFrame,
    ...
    if cn0 > cond_thresh:
        raise RuntimeError(f'Base design ill-conditioned (cond={cn0:.1f}). Consider removing/reducing base.')

    base_cv = _cv_dev_poisson(y, X_base, trial_ids, offset_log, nfolds=nfolds, rng=rng)
    log: List[StepResult] = [StepResult(step=0, added_group=None,
                                        cv_deviance=base_cv, rel_improve=0.0,
    ...
    while step < max_steps and remaining:
        ...
        for g in remaining:
            ...
            if cn > cond_thresh:
                ...
                continue
            score = _cv_dev_poisson(y, X_try, trial_ids, offset_log, nfolds=nfolds, rng=rng)
            trial_scores.append((g, score, cn, X_try.shape[1]))

        # pick best candidate
        g_best, score_best, cn_best, p_best = min(trial_scores, key=lambda t: t[1])
        ...
        log.append(StepResult(step=step, added_group=g_best,
                              cv_deviance=score_best, rel_improve=rel,
